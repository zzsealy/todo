...
const TodoList = () => {
    ...
    const getTodoListHook = () => {
        ...
        const getPath = `${constant.baseUrl}/todo/todo_list/${params.id}/`
        axios.get(getPath, config)
            .then((res) => {
                if (res.data.status_code === 200) {
                    setTodoList(res.data.todo_list)
                    setTodos(res.data.todo_list.child_todo)
                    setDateString(res.data.todo_list.expect_finish_date)
    ...
    const handleClickChangeTodoList = (type, tag=null) => {
        if (type === 'close') { // 关闭
            const todoListPath = `${constant.baseUrl}/todo/todo_list/${params.id}/`
